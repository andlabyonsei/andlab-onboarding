...
1. 작업 **실행 전·후**에 리소스 사용량을 확인한다.
2. GPU 작업이라고 해도 **CPU/RAM을 많이 쓰는 경우**가 많다 — `nvidia-smi`만 보지 말 것.
3. 장시간·대규모 작업은 **사전에 랩원에게 공유**한다.
4. 작업 종료 후 **불필요한 프로세스·세션**을 정리한다.
5. 공용서버1이 바쁘면 **공용서버2**로 작업을 분산한다.
